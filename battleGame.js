...
function attack ( attackingPlayer, defendingPlayer, baseDamage, variableDamage ) {
  let damage = baseDamage + (Math.ceil(Math.random() * variableDamage));
  defendingPlayer.health - damage; //defendingPlayer.health -= damage;???
  return `${attackingPlayer} hit ${defendingPlayer} for ${damage}! Next turn!`
}
*/
...
const attack = (attackingPlayer, defendingPlayer, baseDamage, variableDamage) => (defendingPlayer.health - (baseDamage + (Math.ceil(Math.random() * variableDamage))));
